...
public class RatingDAO {
    ...
    public boolean hasUserRatedJastiperForOrder(Integer userId, Integer jastiperId, Integer titipanId) {
        ...
        String sql = "SELECT COUNT(*) FROM ratiing_user WHERE idUser = ? AND idDriver = ?";
        
        System.out.println("DEBUG: hasUserRatedJastiperForOrder - UserID: " + userId + ", JastiperID: " + jastiperId);
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    System.out.println("DEBUG: Found " + count + " ratings from user " + userId + " to jastiper " + jastiperId);
                    return count > 0;
                }
            }
        } catch (SQLException e) {
